...
# default is RTZ_SN — floor on [0, 1) (≡ TowardNegative there) — chosen so
...
# default RNE_SF: round-to-nearest with tail draws beyond MaxFiniteOf(T)
...
Random.rand(rng::AbstractRNG, ::Random.SamplerTrivial{Random.CloseOpen01{T}}) where {T<:Binary} =
    Convert(T, RTZ_SN, rand(rng, Float64))

# rand([rng], T; projection=RTZ_SN) / randn([rng], T; projection=RNE_SF):
...
@inline Random.rand(rng::AbstractRNG, ::Type{T}; projection::ProjSpec=RTZ_SN) where {T<:Binary} =
    Convert(T, projection, rand(rng, Float64); rng)::T
@inline Random.rand(::Type{T}; projection::ProjSpec=RTZ_SN) where {T<:Binary} =
    rand(default_rng(), T; projection)::T

function Random.randn(rng::AbstractRNG, ::Type{T}; projection::ProjSpec=RNE_SF) where {T<:Binary}
...
@inline Random.randn(::Type{T}; projection::ProjSpec=RNE_SF) where {T<:Binary} =
