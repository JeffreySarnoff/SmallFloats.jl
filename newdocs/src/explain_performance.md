...
Performance advice for SmallFloats.jl is scattered across the User Guide,
...
this path up automatically; NaN sorts last under the default order (first
under `rev=true`), matching Base's own conventions for float sorting so
existing code does not need to special-case `Binary` vectors.
