# Julia Compatibility: the Base Register
...
them (see [Session Defaults](explain_session_defaults.md)):
...
`rand`/`randn` (see [Random Values](explain_random.md)). All of it goes through the same
...
| `widen` | the format's promotion carrier — see [No Automatic Promotion](explain_no_promotion.md) for why it is not another format |
