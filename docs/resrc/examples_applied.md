...
so you can reproduce them exactly.

## Basic

!!! note "Two Basic examples live in the Tutorials"
    *One value, every rounding mode* is in
    [Projection: Rounding & Saturation](concept_rounding_saturation.md), and
    *Enumerating a whole format* is in
    [Values, Code Points & Conversion](concept_values_codepoints.md). They are teaching
