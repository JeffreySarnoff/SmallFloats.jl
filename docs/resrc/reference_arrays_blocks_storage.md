...
Signatures only. For narrative treatment, see the manual's Arrays,
...
| Callable operation object | `Op(:Op, fr, ρ)` then `.(A)` or `map(Op(:Op, fr, ρ), A, B)` | manual |
...
[Operations & Arrays](workflow_arrays.md),
