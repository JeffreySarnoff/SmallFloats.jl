Performance Tips · The Julia Language window.dataLayer = window.dataLayer || \[\]; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'UA-28835595-6', {'page\_path': location.pathname + location.search + location.hash}); documenterBaseURL="../.."
...
*   Manual
    ...
    *   Performance Tips
        *   [Table of contents](#man-performance-tips-toc)
        *   [General advice](#General-advice)
        *   [Type inference](#Type-inference)
        *   [Memory management and arrays](#Memory-management-and-arrays)
        *   [Execution latency, package loading and package precompiling time](#Execution-latency,-package-loading-and-package-precompiling-time)
...
[GitHub](https://github.com/JuliaLang/julia "View the repository on GitHub")[](https://github.com/JuliaLang/julia/blob/master/doc/src/manual/performance-tips.md "Edit source on GitHub")[](# "Settings")[](javascript:; "Collapse all docstrings")

# [Performance Tips](#man-performance-tips)[](#man-performance-tips "Permalink")
...
## [Table of contents](#man-performance-tips-toc)[](#man-performance-tips-toc "Permalink")
...
*   *   [Table of contents](#man-performance-tips-toc)
    ...
    *   *   [Performance critical code should be inside a function](#Performance-critical-code-should-be-inside-a-function)
        ...
        *   [Measure performance with `@time` and pay attention to memory allocation](#Measure-performance-with-[@time]\(@ref\)-and-pay-attention-to-memory-allocation)
    ...
    *   *   [Avoid containers with abstract type parameters](#man-performance-abstract-container)
        ...
        *   [`@code_warntype`](#man-code-warntype)
    ...
    *   [Execution latency, package loading and package precompiling time](#Execution-latency,-package-loading-and-package-precompiling-time)
    *   *   [Reducing time to first plot etc.](#Reducing-time-to-first-plot-etc.)
        *   [Reducing package loading time](#Reducing-package-loading-time)
        *   [Reducing precompilation time](#Reducing-precompilation-time)
    *   [Miscellaneous](#Miscellaneous)
    *   *   [Tweaks](#Tweaks)
...
## [General advice](#General-advice)[](#General-advice "Permalink")

### [Performance critical code should be inside a function](#Performance-critical-code-should-be-inside-a-function)[](#Performance-critical-code-should-be-inside-a-function "Permalink")
...
### [Avoid untyped global variables](#Avoid-untyped-global-variables)[](#Avoid-untyped-global-variables "Permalink")
...
If a non-constant global is known to always be of the same type, [the type should be annotated](../variables-and-scoping/#man-typed-globals); `const` globals need not be annotated because their type is inferred from their initialization value.
...
Note[](#Note-35189990afa5b637 "Permalink")
...
### [Measure performance with](#Measure-performance-with-[@time]\(@ref\)-and-pay-attention-to-memory-allocation) [`@time`](../profile/#@time) and pay attention to memory allocation[](#Measure-performance-with-[@time]\(@ref\)-and-pay-attention-to-memory-allocation "Permalink")
...
For more information about memory management and garbage collection in Julia, see [Memory Management and Garbage Collection](../memory-management/#man-memory-management).
...
### [Break functions into multiple definitions](#Break-functions-into-multiple-definitions)[](#Break-functions-into-multiple-definitions "Permalink")
...
## [Type inference](#Type-inference)[](#Type-inference "Permalink")
...
### [Avoid containers with abstract type parameters](#man-performance-abstract-container)[](#man-performance-abstract-container "Permalink")
...
See also the discussion under [Parametric Types](../types/#Parametric-Types).

### [Avoid fields with abstract type](#Avoid-fields-with-abstract-type)[](#Avoid-fields-with-abstract-type "Permalink")
...
### [Avoid fields with abstract containers](#Avoid-fields-with-abstract-containers)[](#Avoid-fields-with-abstract-containers "Permalink")
...
### [Annotate values taken from untyped locations](#Annotate-values-taken-from-untyped-locations)[](#Annotate-values-taken-from-untyped-locations "Permalink")
...
### [Be aware of when Julia avoids specializing](#Be-aware-of-when-Julia-avoids-specializing)[](#Be-aware-of-when-Julia-avoids-specializing "Permalink")
...
### [Write "type-stable" functions](#Write-"type-stable"-functions)[](#Write-"type-stable"-functions "Permalink")
...
### [Avoid changing the type of a variable](#Avoid-changing-the-type-of-a-variable)[](#Avoid-changing-the-type-of-a-variable "Permalink")
...
### [Separate kernel functions (aka, function barriers)](#kernel-functions)[](#kernel-functions "Permalink")
...
### [](#man-code-warntype)[`@code_warntype`](../../stdlib/InteractiveUtils/#InteractiveUtils.@code_warntype)[](#man-code-warntype "Permalink")
...
### [Performance of captured variable](#man-performance-captured)[](#man-performance-captured "Permalink")
...
### [Types with values-as-parameters](#man-performance-value-type)[](#man-performance-value-type "Permalink")
...
### [The dangers of abusing multiple dispatch (aka, more on types with values-as-parameters)](#The-dangers-of-abusing-multiple-dispatch-\(aka,-more-on-types-with-values-as-parameters\))[](#The-dangers-of-abusing-multiple-dispatch-\(aka,-more-on-types-with-values-as-parameters\) "Permalink")
...
## [Memory management and arrays](#Memory-management-and-arrays)[](#Memory-management-and-arrays "Permalink")

### [Pre-allocate outputs](#Pre-allocate-outputs)[](#Pre-allocate-outputs "Permalink")
...
### [Consider StaticArrays.jl for small fixed-size vector/matrix operations](#Consider-StaticArrays.jl-for-small-fixed-size-vector/matrix-operations)[](#Consider-StaticArrays.jl-for-small-fixed-size-vector/matrix-operations "Permalink")
...
### [More dots: Fuse vectorized operations](#More-dots:-Fuse-vectorized-operations)[](#More-dots:-Fuse-vectorized-operations "Permalink")
...
### [Fewer dots: Unfuse certain intermediate broadcasts](#man-performance-unfuse)[](#man-performance-unfuse "Permalink")
...
### [Access arrays in memory order, along columns](#man-performance-column-major)[](#man-performance-column-major "Permalink")
...
### [Copying data is not always bad](#Copying-data-is-not-always-bad)[](#Copying-data-is-not-always-bad "Permalink")
...
### [Multithreading and linear algebra](#man-multithreading-linear-algebra)[](#man-multithreading-linear-algebra "Permalink")
...
### [Alternative linear algebra backends](#man-backends-linear-algebra)[](#man-backends-linear-algebra "Permalink")
...
## [Execution latency, package loading and package precompiling time](#Execution-latency,-package-loading-and-package-precompiling-time)[](#Execution-latency,-package-loading-and-package-precompiling-time "Permalink")

### [Reducing time to first plot etc.](#Reducing-time-to-first-plot-etc.)[](#Reducing-time-to-first-plot-etc. "Permalink")

The first time a julia method is called it (and any methods it calls, or ones that can be statically determined) will be compiled. The [`@time`](../profile/#@time) macro family illustrates this.
...
When developing a package, you may be able to improve the experience of your users with _precompilation_ so that when they use the package, the code they use is already compiled. To precompile package code effectively, it's recommended to use [`PrecompileTools.jl`](https://julialang.github.io/PrecompileTools.jl/stable/) to run a "precompile workload" during precompilation time that is representative of typical package usage, which will cache the native compiled code into the package `pkgimage` cache, greatly reducing "time to first execution" (often referred to as TTFX) for such usage.
...
### [Reducing package loading time](#Reducing-package-loading-time)[](#Reducing-package-loading-time "Permalink")

Keeping the time taken to load the package down is usually helpful. General good practice for package developers includes:

1.  Reduce your dependencies to those you really need. Consider using [package extensions](../../base/math/#Base.:--Tuple{Any, Any}) to support interoperability with other packages without bloating your essential dependencies.
...
The tool [`@time_imports`](../../stdlib/InteractiveUtils/#Base.@time_imports) can be useful in the REPL to review the above factors.
...
If loading time is dominated by slow `__init__()` methods having compilation, one verbose way to identify what is being compiled is to use the julia args `--trace-compile=stderr --trace-compile-timing` which will report a [`precompile`](../../base/base/#Base.precompile) statement each time a method is compiled, along with how long compilation took. The InteractiveUtils macro [`@trace_compile`](../../stdlib/InteractiveUtils/#Base.@trace_compile) provides a way to enable those args for a specific call. So a call for a complete report report would look like:
...
### [Reducing precompilation time](#Reducing-precompilation-time)[](#Reducing-precompilation-time "Permalink")
...
There are also profiling options such as [using the external profiler Tracy to profile the precompilation process](../../devdocs/external_profilers/#Profiling-package-precompilation-with-Tracy).

## [Miscellaneous](#Miscellaneous)[](#Miscellaneous "Permalink")

### [Tweaks](#Tweaks)[](#Tweaks "Permalink")

These are some minor points that might help in tight inner loops.

*   Avoid unnecessary arrays. For example, instead of [`sum([x,y,z])`](../../base/collections/#Base.sum) use `x+y+z`.
*   Use [`abs2(z)`](../../base/math/#Base.abs2) instead of [`abs(z)^2`](../../base/math/#Base.:^-Tuple{Number, Number}) for complex `z`. In general, try to rewrite code to use [`abs2`](../../base/math/#Base.abs2) instead of [`abs`](../../base/math/#Base.abs) for complex arguments.
...
### [Fix deprecation warnings](#Fix-deprecation-warnings)[](#Fix-deprecation-warnings "Permalink")
...
### [Performance Annotations](#man-performance-annotations)[](#man-performance-annotations "Permalink")

Sometimes you can enable better optimization by promising certain program properties.

*   Use [`@inbounds`](../../base/base/#Base.@inbounds) to eliminate array bounds checking within expressions. Be certain before doing this. If the indices are ever out of bounds, you may suffer crashes or silent corruption.
...
Note[](#Note-bfdafbe19a6e2064 "Permalink")
...
### [Treat Subnormal Numbers as Zeros](#Treat-Subnormal-Numbers-as-Zeros)[](#Treat-Subnormal-Numbers-as-Zeros "Permalink")
...
### [Avoid string interpolation for I/O](#Avoid-string-interpolation-for-I/O)[](#Avoid-string-interpolation-for-I/O "Permalink")
...
### [Avoid eager string materialization](#Avoid-eager-string-materialization)[](#Avoid-eager-string-materialization "Permalink")
...
### [Optimize network I/O during parallel execution](#Optimize-network-I/O-during-parallel-execution)[](#Optimize-network-I/O-during-parallel-execution "Permalink")
