...
The package{\textquotesingle}s speed comes from the first idea{\textquotesingle}s finiteness: an 8-bit unary operation is a function with 256 inputs, so its \emph{entire} behavior is a 256-byte table, built once through the exact scalar path and then gathered per element (0.27 ns). The exactness is inherited: table and scalar are the same function by construction.
