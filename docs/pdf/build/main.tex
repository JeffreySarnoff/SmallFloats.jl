...
\item \textbf{Enumerated where enumeration is affordable, labelled where it is not.} ≈ 35 million verified units across the gates and tiers, each labelled \emph{exhaustive} or \emph{sampled} in a roll-call printed at the end of every run.
...
Nor is \texttt{Binary16p8se} a \texttt{BFloat16}. The exponent biases differ by one, so \emph{every code point denotes a different value}. Converting between them is a conversion, never a reinterpretation. See \href{explain\_\allowbreak{}float16.md}{Why \texttt{Binary16p11se} is not \texttt{Float16}}.
...
\label{4899155108112179322}{}


The documentation is organized by \emph{what you are trying to do}, not by module. Five entrances:
...
\toprule
\textbf{run something now} & \href{installation.md}{Installation} → \href{quickstart.md}{Quickstart} \\
\textbf{understand the ideas} & \href{mentalmodel.md}{Mental Model} — five ideas that carry the whole package \\
\textbf{learn the standard} & \href{fifteenminutes.md}{Introducing P3109} \\
\textbf{do a specific task} & the \href{howto\_\allowbreak{}choose\_\allowbreak{}format.md}{How-To} section — each page is one job \\
\textbf{look something up} & the \href{cheatsheet.md}{Cheat Sheet}, or \href{ref\_\allowbreak{}formats.md}{Reference} \\
\bottomrule
\end{tabulary}
...
\subsection{The sections, and what each is for}



\label{1406093157261069983}{}


\begin{itemize}
\item \textbf{Getting Started} — installation, a first session, and the two orientation pages. Read \texttt{Mental Model} once, slowly; the rest of the docs become reference material rather than prose to wade through.


\item \textbf{Tutorials} — five sequenced lessons that build on each other: values, projection, arrays, blocks, stochastic rounding. Follow them in order the first time.


\item \textbf{How-To} — task-shaped answers. {\textquotedbl}Choose a format{\textquotedbl}, {\textquotedbl}quantize a tensor{\textquotedbl}, {\textquotedbl}make stochastic rounding reproducible{\textquotedbl}. Independent of each other; go straight to the one you need.


\item \textbf{Explanations} — the \emph{why} behind decisions that surprise people: the projection contract, session defaults, \texttt{Float16} non-equivalence, the absence of automatic promotion, the performance model.


\item \textbf{Reference} — the API surface: formats, projections, operations, defaults, arrays and blocks, conformance, the Base register, and the docstrings.


\item \textbf{Foundations} — the code-point algebra the whole package computes on, stated as verified identities.


\item \textbf{Examples} — complete runnable sessions with their output, split into applied (public API) and internals (unexported machinery).


\item \textbf{Insights} — how it works inside: architecture, the engine, the oracle, tables, blocks, and the verification and benchmark doctrines. Also where to go to extend the package.


\item \textbf{Support} — troubleshooting by symptom, glossary, benchmarks.

\end{itemize}


\begin{tcolorbox}[toptitle=-1mm,bottomtitle=1mm,colback=admonition-tip!50!white,colframe=admonition-tip,title=\textbf{The one rule that catches everyone}]
An \texttt{Unsigned} argument always means a \textbf{code point}; every other \texttt{Real} means a \textbf{value}. \texttt{T(0x08)} and \texttt{T(8.0)} are different things by design, and \texttt{T(8)} is the value 8.0 — the signedness of the integer type is what distinguishes them. This holds at every bitwidth.
...
\section{Choose how values print}



\label{14626719229294664551}{}


Throughout this documention, most examples show their results in a fully type-annotated manner, using the style \texttt{:typed}. This is one of four available styles.



The styles, all showing the same value \texttt{Binary8p4se(1.6)}:
...
\texttt{:value} is the package default — a \texttt{Binary} prints as the number it denotes, which is what ordinary output and generic Julia code want.



Set the process-wide style with \href{@ref}{\texttt{set\_\allowbreak{}show\_\allowbreak{}style!}}, and query it with \hyperlinkref{16179625954962935066}{\texttt{get\_\allowbreak{}show\_\allowbreak{}style}}:
...
The style applies wherever a value prints, containers included — which is where it earns its keep:
...
\texttt{VALID\_\allowbreak{}SHOW\_\allowbreak{}STYLES} lists the four, and an unknown style is refused rather than silently ignored:
...
\begin{tcolorbox}[toptitle=-1mm,bottomtitle=1mm,colback=admonition-tip!50!white,colframe=admonition-tip,title=\textbf{Scoped rather than global}]
\texttt{set\_\allowbreak{}show\_\allowbreak{}style!} is process-wide and stays set. To change the style for one piece of output without disturbing anything else, put it on the \texttt{IO} — an \texttt{:binary\_\allowbreak{}show\_\allowbreak{}style} property overrides the process default for that stream only, and \texttt{get\_\allowbreak{}show\_\allowbreak{}style(io)} reports what a given stream resolves to:
...
This is also the form to prefer under concurrency: the process-wide setter is shared mutable state, an \texttt{IOContext} is not.
...
\section{A format is a finite set of datums}
...
\section{A value is a wrapper around a code point; decode is exact}
...
\section{Every computation is exact math, then one projection}
...
\section{Projection = rounding mode × saturation mode}
...
\section{Approximation is opt-in, named, and measured}
...
\textbf{Where to go from here.} To build the ideas into skills: Values, Code Points \& Conversion. To look something up: the Specifics pages. To understand \emph{why} the package is built this way: The Projection Contract and the rest of the Explanations section.
...
\href{mentalmodel.md}{Mental Model}, \href{papers\_\allowbreak{}index.md}{The Standard and this Design}, \href{quickstart.md}{Quickstart}.
...
\chapter{Values, Code Points \& Conversion}
...
\section{Four ways in, two ways out}
...
\section{Important: \texttt{Unsigned} means code point, at every width}
...
\section{\texttt{Convert} versus \texttt{convert}}
...
\section{The decode / codepoint round-trip}
...
\section{Enumerating a whole format}
...
\section{Stepping and classification}
...
\section{\texttt{zero}, \texttt{one}, \texttt{eps}, \texttt{typemax}}
...
\chapter{Projection: Rounding \& Saturation}
...
\section{One value, every rounding mode}
...
\section{Session defaults, used safely}
...
\section{First contact with stochastic rounding}
...
\section{The two registers, again}
...
\section{Array calls: \texttt{Add(T, ρ, A, B)}, \texttt{Exp(T, ρ, A)}}
...
\section{Watching a table build}
...
\section{\texttt{vmap} and \texttt{vmap!}}
...
\section{The \texttt{Op} callable, with \texttt{map} and broadcast}
...
\texttt{TotalOrder(x, y)} exposes the draft{\textquotesingle}s total order directly if you need the comparison itself rather than a sorted array — single NaN smallest, so \texttt{sort} places NaN first, not last as with \texttt{Float64}.
...
\chapter{Blocks \& MX-Style Quantization}
...
\section{\texttt{ConvertToBlockMaxAbsFinite}}
...
\section{\texttt{ConvertFromBlock}}
...
\section{The stage-wide pitfall}
...
\section{\texttt{BlockDotProduct}: one rounding, at the end}
...
\section{Unbiasedness: nearest is not, stochastic is (on average)}
...
\section{The R-sweep: auditing the distribution exactly}
...
\section{The payoff: swamping and stall}
...
\href{howto\_\allowbreak{}choose\_\allowbreak{}format.md}{How-Tos}, \href{ref\_\allowbreak{}formats.md}{Format Names \& Queries}, \href{ref\_\allowbreak{}projections.md}{Projection Specifications}.
...
\chapter{Quantize a Tensor or Model}
...
\chapter{Use Blocks to Track Dynamic Range}
...
\chapter{Get Reproducible Stochastic Results}
...
\chapter{Work with Packed Storage}
...
\chapter{Convert To/From Float16 \& BFloat16 Safely}
...
\chapter{Register \& Measure an Approximation (κ)}
...
\chapter{Read \& Export the Conformance Declaration}
...
\chapter{Formats, Aliases \& Representations}
...
\section{Why 504 formats, and why only 120 are exported}
...
\chapter{The Projection Contract}
...
\section{What {\textquotedbl}exact{\textquotedbl} means, per carrier}
...
\section{Why \texttt{Rational} inputs are refused, not double-rounded}
...
\section{Why the contract is a single, named function}
...
\chapter{Session Defaults \& Their Scope}
...
\section{Coherence in both directions}
...
\section{The defaults are process-global}
...
\section{The \texttt{with\_\allowbreak{}default\_\allowbreak{}*} combinators}
...
\subsection{The speculation-guard cost model}
...
\subsection{Allocation contract}
...
\section{There is no accumulator default}
...
\chapter{Binary16p11se Is Not Float16}
...
\section{The bias is the key difference}
...
\section{Single NaN, no −0, domain as a parameter}
...
\section{Convert, never reinterpret}
...
\section{Why P3109 chose a different bias rule at all}
...
\section{Interop via \texttt{datumsexact}}
...
\chapter{Why No Cross-Format Promotion}
...
\section{Mixed \texttt{Binary} formats: deliberate failure, not silent widening}
...
\section{\texttt{Binary} with ordinary numbers: promotes to the promotion carrier}
...
\section{Why widen is not another format}
...
\section{Why this differs from \texttt{Int}/\allowbreak\texttt{Float64} promotion}
...
\section{Explicit \texttt{Convert} as the answer}
...
\section{What this buys you as a caller}
...
\chapter{Random Values: What the Draws Mean}
...
\section{Setup 1 — quick and implicit}
...
\section{Setup 2 — an explicit RNG stream}
...
\section{Setup 3 — arrays and in-place}
...
\section{Setup 4 — choosing the projection}
...
\section{Why floor for \texttt{rand} but nearest for \texttt{randn}}
...
\section{Why this matters for the smallest formats}
...
\section{Pass formats statically}
...
\section{Convenience forms are free at the initial default}
...
\section{Bulk work belongs in array calls}
...
\subsection{Ternary tiers scale with bitwidth}
...
\section{Stochastic array calls are always per-element}
...
\section{Memory: \texttt{PackedVector} and \texttt{BlockVector}}
...
\section{Sorting: O(n) counting sort}
...
\section{Do not use \texttt{@fastmath}}
...
\chapter{Format Names \& Queries}



\label{7338399722403808079}{}
...
\needspace{19\baselineskip}
\section{\texttt{format()} and opt-in access}
...
\section{Value-and-type call forms}
...
\section{\texttt{ProjSpec} constructor}
...
\section{Deterministic predefined specs (6 × 3)}
...
\needspace{17\baselineskip}
\section{Saturation-mode meaning}
...
\section{Explicit \texttt{R} range rule}
...
\needspace{18\baselineskip}
\section{Base spellings that are componentwise composites}
...
\section{Deliberately unmapped draft operations}
...
\chapter{Session Defaults}



\label{11293141629967711570}{}
...
\needspace{21\baselineskip}
\section{\texttt{with\_\allowbreak{}default\_\allowbreak{}*} combinators}
...
\chapter{Arrays, Blocks \& Packed Storage}



\label{6261291168643383015}{}
...
\needspace{16\baselineskip}
\section{Table cache introspection}
...
\needspace{18\baselineskip}
\subsection{Generated \texttt{BlockOp} / \texttt{ScaledOp} pattern}
...
\chapter{Julia Compatibility: the Base Register}



\label{13505593374100832066}{}
...
Because the veneers follow the session default projection, changing it changes them (see \href{explain\_\allowbreak{}session\_\allowbreak{}defaults.md}{Session Defaults}):
...
\texttt{widen} & the format{\textquotesingle}s promotion carrier — see \href{explain\_\allowbreak{}no\_\allowbreak{}promotion.md}{No Automatic Promotion} for why it is not another format \\
...
\chapter{Conformance \& Approximation Registry}
...
\section{Understatement-rejection rule}
...
\section{κ = NaN acknowledgment rule}
...
\href{howto\_\allowbreak{}conformance.md}{Read \& Export the Conformance Declaration}, \href{under\_\allowbreak{}verification.md}{Package Verification \& Benchmarking}.
...
\chapter{Formal Foundations: the Code-Point Algebra}



\label{11419846982893134574}{}
...
It is the layer \emph{below} \href{explain\_\allowbreak{}formats.md}{Formats, Aliases \& Representations}. Read that page for what a format is and how to name one; read this page when you need to compute with code points, prove something about them, or check the package against an independent derivation.
...
P3109 uses \texttt{B = 2{\textasciicircum}(Ebits−1)}, where IEEE-754 uses \texttt{2{\textasciicircum}(Ebits−1) − 1}. The difference of one is exactly why \texttt{Binary16p11se} is not \texttt{Float16} — every code point denotes a different value. See \href{explain\_\allowbreak{}float16.md}{Why \texttt{Binary16p11se} is not \texttt{Float16}}.

\end{tcolorbox}


\needspace{33\baselineskip}
\section{The NaN anchor}
...
\section{Decoding: code to value}
...
Which is why \texttt{sort} on a \texttt{Binary} vector puts NaN \textbf{first}, not last as \texttt{Float64} sorting does. See \href{ref\_\allowbreak{}julia\_\allowbreak{}compat.md}{Julia Compatibility} for what that means for generic code.



\section{Sign, magnitude, negation}
...
\section{Checking your own derivation}
...
The exhaustive form of that check — every distinguished code of every one of the 504 formats, plus the decoding formula at every positive finite code point — is in \href{examples\_\allowbreak{}internals.md}{Worked Examples: Internals}.
...
\begin{itemize}
\item \href{explain\_\allowbreak{}formats.md}{Formats, Aliases \& Representations} — what a format is, and the type-vs-alias distinction.


\item \href{under\_\allowbreak{}engine.md}{The Encoding \& Projection Engine} — how the package walks this algebra at speed.


\item \href{explain\_\allowbreak{}float16.md}{Why \texttt{Binary16p11se} is not \texttt{Float16}} — the bias difference, stated in these terms.


\item \href{ref\_\allowbreak{}formats.md}{Format Queries} — the package{\textquotesingle}s accessors for every symbol named here.
...
\item \textbf{\href{examples\_\allowbreak{}internals.md}{Internals}} — unexported machinery. Pipeline introspection, exhaustive self-verification, κ measurement, benchmarking. Stable enough to learn from, not covered by semver.
...
\toprule
\hyperlinkref{14660386703135810651}{One value, every rounding mode} & the six deterministic μ side by side on one between-grid value \\
...
Task-shaped instructions, as opposed to worked demonstrations, are in the How-To section — start at \href{howto\_\allowbreak{}choose\_\allowbreak{}format.md}{Choosing a Format}. The difference: a how-to tells you what to do, an example shows a complete session with its output and discusses what the output means.



\chapter{Worked Examples: Applied}



\label{2433275757356789780}{}
...
\begin{tcolorbox}[toptitle=-1mm,bottomtitle=1mm,colback=admonition-note!50!white,colframe=admonition-note,title=\textbf{Two Basic examples live in the Tutorials}]
\emph{One value, every rounding mode} is in \href{tutorial2\_\allowbreak{}projection.md}{Projection: Rounding \& Saturation}, and \emph{Enumerating a whole format} is in \href{tutorial1\_\allowbreak{}values.md}{Values, Code Points \& Conversion}. They are teaching material first; this page does not restate them.
...
\chapter{Worked Examples: Internals}



\label{9519708165358832524}{}
...
\subsection{Symbolic sticky: projecting just below a number}
...
On the host recorded in the \href{benchmarks.md}{benchmark report}, the corresponding in-domain \texttt{Add} row has a 9.0 ns median and zero allocations. Treat that as a check on the methodology, not a portable expected value: the tuple printed by this snippet depends on your CPU, Julia build, and benchmark environment.
...
\subsection{Checking the code-point algebra against the package}
...
The identities on \href{formal\_\allowbreak{}codepoints.md}{Formal Foundations} are claims about every format, so they are checked over every format — the distinguished codes for all 504, and the decoding formula at every positive finite code point of each.
...
\href{under\_\allowbreak{}oracle.md}{The Oracle \& Rigor Classes}, \href{under\_\allowbreak{}tables.md}{Tables, Kernels \& Sorting}, \href{examples\_\allowbreak{}internals.md}{Worked Examples: Internals}.
...
\href{under\_\allowbreak{}engine.md}{The Encoding \& Projection Engine}, \href{under\_\allowbreak{}oracle.md}{The Oracle \& Rigor Classes}, \href{examples\_\allowbreak{}internals.md}{Worked Examples: Internals}.



\chapter{Blocks: Exactness without a Superaccumulator}



\label{7475629752098886212}{}
...
\section{Step 1: decode each lane exactly}
...
\section{Step 2: accumulate exactly — two strategies}
...
\needspace{22\baselineskip}
\subsection{The fast path: a span filter}
...
\subsection{Why the filter is sound}
...
\subsection{The fallback: precision derived from the formats}
...
\section{Why no superaccumulator}
...
\begin{itemize}
\item \href{ref\_\allowbreak{}arrays\_\allowbreak{}blocks.md}{Blocks \& Scaled Operations} — the API.


\item \href{tutorial4\_\allowbreak{}blocks.md}{Working with Blocks} — the tutorial.


\item \href{howto\_\allowbreak{}blocks\_\allowbreak{}dynamic\_\allowbreak{}range.md}{Blocks for Dynamic Range} — when to reach for one.


\item \href{under\_\allowbreak{}oracle.md}{The Oracle \& Rigor Classes} — where the enclosure cascade comes from.


\item \hyperlinkref{13954472669056568806}{Proving your fused kernel exact} — \texttt{BlockDotProduct} against 512-bit truth.


\item \href{howto\_\allowbreak{}add\_\allowbreak{}operation.md}{Add an Operation} — adding block variants.

\end{itemize}


\chapter{Package Verification \& Benchmarking}



\label{4143258828077855129}{}
...
\section{The Approach to Benchmarking}
...
\section{Audit stochastic rounding}
...
\section{Measuring Approximation}
...
\href{howto\_\allowbreak{}add\_\allowbreak{}operation.md}{Add an Operation}, \href{howto\_\allowbreak{}benchmark.md}{Benchmark Correctly}, \href{examples\_\allowbreak{}internals.md}{Worked Examples: Internals}.
...
\section{Set up a measurement that is actually measuring your code}
...
A compact reference for the SmallFloats.jl operations you are most likely to write. For semantics and explanation, follow the links out of each section; for implementation detail, see \href{under\_\allowbreak{}architecture.md}{Insights}.
...
Common Base spellings under the session projection (initially \texttt{RNE\_\allowbreak{}SN}) — the full register is in \href{ref\_\allowbreak{}julia\_\allowbreak{}compat.md}{Julia Compatibility}:
...
Symptom-first versions of these, with the error text you would actually see, are in \href{troubleshooting.md}{Troubleshooting}.
...
The reasoning behind each is in the \href{explain\_\allowbreak{}performance.md}{Performance Model}; the measurement rules are in \href{under\_\allowbreak{}verification.md}{Package Verification \& Benchmarking}.
...
For worked applications, continue to the \href{examples\_\allowbreak{}index.md}{Examples Index}. For correctness and benchmark methodology, continue to \href{under\_\allowbreak{}architecture.md}{Architecture}.
...
\href{cheatsheet.md}{Cheat Sheet}, \href{glossary.md}{Glossary}, \href{explain\_\allowbreak{}performance.md}{Performance Model}, \href{under\_\allowbreak{}verification.md}{Package Verification \& Benchmarking}.



\chapter{The Standard and this Design}



\label{15888356242149815834}{}
...
\section{Deigning to the Draft Standard}



\label{13956872479192544617}{}
...
Order key & An integer, sign-magnitude-folded encoding of a value used for comparisons and sorting — monotone with the total order, NaN mapped to key \texttt{0} (below every datum, per §4.12.1), enabling O(n) counting sort. & The Encoding \& Projection Engine \\
...
Total order & The draft{\textquotesingle}s total ordering over a format{\textquotesingle}s value set, with the single NaN placed first, below −Inf (§4.12.1); the basis for \texttt{TotalOrder}, \texttt{isless}, comparisons, and counting sort. & The Encoding \& Projection Engine \\
\bottomrule
\end{tabulary}
