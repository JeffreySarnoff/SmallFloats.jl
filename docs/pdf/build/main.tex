...
Bit-exact defined results on every default path; one projection engine as the single write path into a code point; approximation only behind an explicitly named, exhaustively measured κ registry; \textbf{≈ 35 million verified units across 13 gates and tiers}, each labelled exhaustive or sampled in a roll-call printed at the end of every run; table-gather kernels at fractions of a nanosecond per element.
...
\textbf{Most format names are opt-in.} \texttt{using SmallFloats} exports the \textbf{120 aliases at K ≤ 8}. The other 384 arrive with \texttt{using SmallFloats.Formats}, and are reachable without it as \texttt{SmallFloats.Binary16p6se} or \texttt{format(\allowbreak{}16, 6, true, true)}.



\textbf{\texttt{Binary16p11se} is not \texttt{Float16}} (nor is \texttt{Binary16p8se} a \texttt{BFloat16}). The exponent biases differ by one, so \emph{every code point denotes a different value}. Converting between them is a conversion, never a reinterpretation — the \hyperlinkref{10272851679537904033}{User Guide} has the full comparison.
...
SmallFloats.jl is a conforming, performance-oriented Julia implementation of the \textbf{IEEE P3109 draft standard} — \emph{Arithmetic Formats for Machine Learning} — covering every binary format the draft defines at bitwidths 3 through 16, every projection (rounding × saturation) mode including the three stochastic families, the full scalar operation catalog, block-scaled ({\textquotedbl}MX-style{\textquotedbl}) operations, and the draft{\textquotesingle}s conformance and κ-approximation machinery.
...
\item \textbf{Approximation is opt-in, named, and measured.} Faster-but-inexact implementations live behind an explicit registry (\hyperlinkref{852197704347416372}{\texttt{register\_\allowbreak{}approx!}}); each registration{\textquotesingle}s deviation bound κ is \emph{measured by exhaustive enumeration} at registration time, and understated declarations are rejected. Nothing approximate is reachable from the default API.


\item \textbf{Enumerated where enumeration is affordable, and labelled where it is not.} The code lattice is swept \textbf{exhaustively at every bitwidth} — all 504 formats, 7 602 160 code points — and the projection engine is checked against an independent \texttt{Rational\{BigInt\}} reference over every realized \texttt{(P, B)} cell. Where a space is too large to enumerate (the ordered-pair cross-product above K = 8 is 4.3 × 10⁹ pairs for a single format), the suite says \textbf{{\textquotedbl}sampled{\textquotedbl}} in those words rather than rounding up. Every run ends with a coverage roll-call naming each gate, what it compared, and which of the two it was: ≈ 35 million verified units across 13 gates and tiers.


\item \textbf{Fast where it matters.} Pure-mode elementwise work runs through precomputed lookup tables (sub-nanosecond per element); the scalar path is fully specialized and allocation-free (\texttt{decode} ≈ 1.2 ns, \texttt{project} ≈ 6.6 ns, a complete \texttt{Add} ≈ 9 ns including projection); sub-byte packed storage, integer-keyed ordering with an O(n) counting sort, and a mask-based rounding core round out the performance story. A reproducible Chairmarks benchmark suite ships in \texttt{benchmarking/}, and every number here comes from it.
...
A format is the type \texttt{Binary\{K,P,SGN,EXT\}}: bitwidth \texttt{K ∈ 3:16}, precision \texttt{P ∈ 1:K} (with the signedness constraint), signed/unsigned \texttt{SGN}, and extended (has ±Inf) or finite \texttt{EXT}. All \textbf{504} draft formats carry their draft names as aliases. The \textbf{120 at K ≤ 8 are exported} by \texttt{using SmallFloats}; the other 384 are opt-in via \texttt{using SmallFloats.Formats}, and are always reachable as \texttt{SmallFloats.Binary16p6se} or programmatically as \texttt{format(\allowbreak{}16, 6, true, true)}.



\texttt{Binary8p4se} \emph{represents} \texttt{Binary\{8,4,true,true\}} ({\textquotedbl}K = 8, P = 4, signed, extended{\textquotedbl}); the trailing letters are \texttt{s}/\allowbreak\texttt{u} for signedness and \texttt{e}/\allowbreak\texttt{f} for extended/finite. Note that the two are related by \texttt{<:}, not \texttt{===} — \texttt{Binary} is abstract (a format is {\textquotedbl}a datum set and an encoding{\textquotedbl}), and the alias names its concrete \emph{representation}.



Values are immutable wrappers around their code point: one byte at K ≤ 8, two above it. Every format has a single NaN and no negative zero. \texttt{Float64} is the exact interchange carrier for the 432 formats whose exponent range it holds; the remaining 72 use \texttt{Float128} or an exact dyadic carrier, selected by the format and never by the caller.
...
Append \texttt{\#main} (or a tag/commit) to pin a revision. For an editable install, \texttt{pkg> dev https:/\allowbreak{}/\allowbreak{}github.com/\allowbreak{}JeffreySarnoff/\allowbreak{}SmallFloats.jl} clones into \texttt{{\textasciitilde}/.julia/dev}, or \texttt{pkg> dev path/\allowbreak{}to/\allowbreak{}SmallFloats.jl} uses an existing clone.



Requires Julia 1.12. \texttt{Quadmath} (libquadmath{\textquotesingle}s \texttt{Float128}) is a hard dependency used only inside the oracle and exact-fallback paths; on platforms where it misbehaves, \texttt{ENV[{\textquotedbl}SmallFloats\_\allowbreak{}Float128{\textquotedbl}] = {\textquotedbl}disable{\textquotedbl}} before loading selects the pure-MPFR configuration with \textbf{identical results} (this equivalence is itself part of the test suite).
...
\texttt{conformance()} returns the live, serializable conformance declaration (formats, operations, modes, instantiated table specializations, and κ registrations); \texttt{SmallFloats.draft\_\allowbreak{}revision(\allowbreak{})} names the implemented draft revision. Interpretations made where the draft text under-determined behavior are marked \texttt{[interp]} in the source comments.
...
The parametric type \texttt{Binary\{K,P,SGN,EXT\}} defines a format: bitwidth \texttt{K ∈ 3:16}, precision \texttt{P} (significand bits including the implicit bit), \texttt{SGN::Bool} for signed/unsigned, \texttt{EXT::Bool} for extended (with ±Inf) versus finite. Every legal combination — \textbf{504 formats} — has a draft-named alias. \texttt{using SmallFloats} exports the \textbf{120 at K ≤ 8}; the rest are one opt-in away:
...
\texttt{Binary8p4se === Binary\{8,4,true,true\}} is \textbf{\texttt{false}}, and the distinction is the draft{\textquotesingle}s own: a \emph{format} is a datum set and an encoding, while the width of the machine word carrying a code point is a representation choice below the standard{\textquotesingle}s level of description. \texttt{Binary} is abstract; \texttt{Binary8p4se} is its concrete representation and is what an array element type or a constructor target needs. Use the alias, or \texttt{format(K, P, Σ, Δ)} when the parameters are runtime values.



The suffix reads: \texttt{p4} precision 4, \texttt{s}/\allowbreak\texttt{u} signed/unsigned, \texttt{e}/\allowbreak\texttt{f} extended/finite.
...
\section{\texttt{Binary16p11se} is not \texttt{Float16}, and \texttt{Binary16p8se} is not \texttt{BFloat16}}



\label{6433558418335013253}{}


The K ≤ 16 grid contains two formats that look like the IEEE-754 \texttt{binary16} and the bfloat16 used in machine learning. \textbf{They are not those types}, and the differences are the kind that produce correct-looking numbers rather than errors.
...
The second line is the trap in one expression. \texttt{Float16(1.5)} has the bit pattern \texttt{0x3e00}; read as a \texttt{Binary16p11se} those same bits denote \textbf{0.75}, because the exponent bias differs by one. Nothing errors, nothing warns, and the answer is off by a factor of two. Always convert.



The remaining rows are draft decisions rather than accidents. A single NaN encoding means there is no payload to propagate and no quiet/signalling distinction; the 2045 code points IEEE spends on NaN payloads are datums here. No negative zero means \texttt{-0.0} and \texttt{0.0} are the same code point, so a sign-symmetric format has an odd number of finite values. And the domain parameter — \texttt{e} for extended (with ±Inf), \texttt{f} for finite — has no IEEE analogue at all: \texttt{Binary16p11sf} spends the two infinity code points on finite values instead.



If you want IEEE semantics, use \texttt{Float16} and \texttt{BFloat16}; the package converts to and from both, exactly where the datum sets permit (\texttt{datumsexact}).
...
A value is an immutable wrapper around its code point — \textbf{one byte at K ≤ 8, two above it}. The storage unit is \texttt{codeunit\_\allowbreak{}type(T)}, a property of the \emph{representation}; the code point occupies the low \texttt{K} bits and the high bits are maintained zero. Four ways in, two ways out:
...
Standard predicates (\texttt{isnan}, \texttt{isinf}, \texttt{isfinite}, \texttt{iszero}, \texttt{signbit}, \texttt{issubnormal}), \texttt{zero}/\allowbreak\texttt{one}/\texttt{eps}/\allowbreak\texttt{typemin}/\texttt{typemax}, and stepping via \texttt{NextGreaterThan} / \texttt{NextLessThan} (the draft{\textquotesingle}s Next operations; \texttt{nextfloat}/\allowbreak\texttt{prevfloat} alias them) all work:
...
\textbf{Setup 3 — arrays and in-place.} \texttt{rand(T, dims...)} / \texttt{randn(T, dims...)} build \texttt{Array\{T\}}; \texttt{rand!(A)} / \texttt{randn!(A)} fill an existing array (one byte per element at K ≤ 8, two above — cheap to preallocate and reuse):
...
Feed array draws straight into the storage layers when that is the goal: \texttt{PackedVector(\allowbreak{}rand(\allowbreak{}Binary4p2se, n))}.
...
A stochastic projection draws its random bits from the \emph{same} rng, so seeded streams stay reproducible. The defaults are the contract-keepers (floor for \texttt{rand}, nearest + \texttt{SatFinite} for \texttt{randn}); opting out can produce \texttt{1.0} from \texttt{rand} or ±Inf/NaN from \texttt{randn}. The array and \texttt{!} forms always use the defaults — for arrays under another projection, draw scalars: \texttt{[rand(\allowbreak{}rng, T; projection = ρ) for \_\allowbreak{} in 1:n]}. Worked transcripts, including the K = 3 tail-clamp comparison: \hyperlinkref{7866567007412100191}{User Examples}.
...
Every deterministic (rounding, saturation) pairing is exported as a constant, named \texttt{R<mode>\_\allowbreak{}Sat<mode>}:
...
\toprule
\texttt{NearestTiesToEven} & \texttt{RNE\_\allowbreak{}SF} & \texttt{RNE\_\allowbreak{}SP} & \texttt{RNE\_\allowbreak{}SN} \\
\texttt{NearestTiesToAway} & \texttt{RNA\_\allowbreak{}SF} & \texttt{RNA\_\allowbreak{}SP} & \texttt{RNA\_\allowbreak{}SN} \\
\texttt{TowardPositive} & \texttt{RTP\_\allowbreak{}SF} & \texttt{RTP\_\allowbreak{}SP} & \texttt{RTP\_\allowbreak{}SN} \\
\texttt{TowardNegative} & \texttt{RTN\_\allowbreak{}SF} & \texttt{RTN\_\allowbreak{}SP} & \texttt{RTN\_\allowbreak{}SN} \\
\texttt{TowardZero} & \texttt{RTZ\_\allowbreak{}SF} & \texttt{RTZ\_\allowbreak{}SP} & \texttt{RTZ\_\allowbreak{}SN} \\
\texttt{ToOdd} & \texttt{RTO\_\allowbreak{}SF} & \texttt{RTO\_\allowbreak{}SP} & \texttt{RTO\_\allowbreak{}SN} \\
\bottomrule
\end{tabulary}
...
The stochastic families are parameterized by the random-bit budget \texttt{N}, so their predefined forms are \emph{constructors} rather than constants: \texttt{RSA\_\allowbreak{}*} for \texttt{StochasticA}, \texttt{RSB\_\allowbreak{}*} for \texttt{StochasticB}, \texttt{RSC\_\allowbreak{}*} for \texttt{StochasticC}, each crossed with the three saturation modes. Call them with the budget — or without, for the default \texttt{N = 8} (\texttt{SmallFloats.DEFAULT\_\allowbreak{}RBITS}):
...
\texttt{RNE\_\allowbreak{}SN} is the package{\textquotesingle}s \emph{initial} default spec. \texttt{default\_\allowbreak{}projspec} reads the session default (see \textbf{Session defaults} below), so every Base-register operator, the same-format convenience methods, and \texttt{T(x::Real)} construction follow \texttt{DefaultProjection()} — \texttt{RNE\_\allowbreak{}SN} until you change it.
...
Prefer \hyperlinkref{11729885955705173533}{\texttt{with\_\allowbreak{}default\_\allowbreak{}projection}} (and \texttt{with\_\allowbreak{}default\_\allowbreak{}type}, \texttt{with\_\allowbreak{}default\_\allowbreak{}returntype}), which restore on exit even if the body throws:
...
Every example below assumes the pristine \texttt{(\allowbreak{}NearestTiesToEven, SatNone)}, which is why each demonstration above restores it explicitly.
...
\texttt{DefaultAccumulatorType} existed through v0.1.0 and was removed. The wide-precision accumulator in a reduction is not a preference: the span filter measures the operands and picks the cheapest carrier that is \emph{provably exact} for them, so both branches return the defined result. A knob overriding that would make \texttt{BlockDotProduct} inexact from the default API. To trade speed for nothing else, set \texttt{ENV[{\textquotedbl}SmallFloats\_\allowbreak{}Float128{\textquotedbl}] = {\textquotedbl}disable{\textquotedbl}}, which is bit-identical by construction.
...
Semantics highlights (all per the draft): \texttt{x/0 → NaN} for every \texttt{x} including ±Inf; \texttt{Recip(±Inf) = 0}; \texttt{0·∞ → NaN}; a NaN operand generally propagates, except the \texttt{*Number}/\allowbreak\texttt{*Finite} extremum variants which prefer non-NaN (and finite) operands; trig of ±Inf is NaN; the π-scaled family reduces exactly mod 2 first.
...
(In that line \texttt{Op} stands for any operation name — \texttt{Add}, \texttt{Exp}, and so on. There is also a \emph{type} called \hyperlinkref{9195041457639280132}{\texttt{Op}}, introduced below, which turns an operation into a callable so \texttt{map} and broadcast work directly.)
...
Inspect or reset the cache — unary/binary and ternary alike — with \texttt{table\_\allowbreak{}bytes()} and \texttt{empty\_\allowbreak{}tables!()}. The ternary policy thresholds and the threading cutoff are internal \texttt{Ref}s (\texttt{SmallFloats.TERNARY\_\allowbreak{}EAGER\_\allowbreak{}BITS} and neighbors in \texttt{tables.jl} and \texttt{kernels.jl}) for the rare case you need to tune them.
...
\subsection{\texttt{map}, broadcast, and the \texttt{Op} callable}



\label{7105487804729200423}{}


\texttt{vmap!} is the in-place, table-aware kernel and is what you want when you have a destination to fill. When you want Julia{\textquotesingle}s own array verbs, bind the operation, result format and projection into an \hyperlinkref{9195041457639280132}{\texttt{Op}} and use it as a function:
...
\texttt{Op} is a singleton type — the operation, format and projection all live in its type parameters — so a call specializes exactly as \texttt{Add(\allowbreak{}Binary8p4se, RNE\_\allowbreak{}SN, x, y)} does and allocates nothing beyond the array \texttt{map} itself builds.
...
\label{8567984038512272605-dup1}{}
...
\texttt{conformance()} returns the live declaration — formats, operations, mode vocabulary, the table specializations actually instantiated this session, and every registered approximation. \texttt{conformance\_\allowbreak{}report()} prints it; \texttt{conformance\_\allowbreak{}dict()} returns a plain nested \texttt{Dict} for JSON/TOML serialization.
...
Declaring \texttt{κ} smaller than the measured deviation throws; NaN-mismatching implementations must be acknowledged with an explicit \texttt{κ = NaN}. Retrieve with \texttt{approx(:my\_\allowbreak{}fast\_\allowbreak{}exp)}, list with \texttt{list\_\allowbreak{}approx()}, and measure anything yourself with \texttt{measure\_\allowbreak{}kappa} / \texttt{codedistance}.
...
\begin{tcolorbox}[toptitle=-1mm,bottomtitle=1mm,colback=admonition-warning!50!white,colframe=admonition-warning,title=\textbf{The abstract format as an array element type}]
\texttt{Binary\{K,P,Σ,Δ\}} is \textbf{abstract}, so an array declared with it has a non-\texttt{isbits} element type and boxes every element:
...
Nothing errors — every operation still returns the right answer, because the package normalizes the abstract format at each constructor and entry point. You simply lose the entire performance story, silently. That makes this the sharper edge of the \texttt{===} distinction: the type comparison is visibly \texttt{false}, while this one costs you and says nothing.

\texttt{similar(a, T)} normalizes the request; \texttt{Vector\{T\}(undef, n)} cannot be intercepted. Use the alias, or \texttt{format(K, P, Σ, Δ)} for runtime parameters.
...
\item The reproducible benchmark suite lives at \texttt{benchmarking/\allowbreak{}benchmarking.jl} and generates a full markdown report for your machine.
...
\texttt{ENV[{\textquotedbl}SmallFloats\_\allowbreak{}Float128{\textquotedbl}] = {\textquotedbl}disable{\textquotedbl}} (set before \texttt{using SmallFloats}) disables the internal \texttt{Float128} fast paths in favor of pure MPFR — results are bit-identical (tested); only build/oracle speed changes.
...
Beyond arithmetic, \texttt{Binary} integrates where the other layers provide it: predicates and constants (\texttt{isnan}, \texttt{isfinite}, \texttt{signbit}, \texttt{zero}, \texttt{one}, \texttt{eps}, \texttt{typemin}/\allowbreak\texttt{typemax}, \texttt{floatmin}/\allowbreak\texttt{floatmax}), comparisons and \texttt{isless} on integer order keys, \texttt{sort} via an O(n) counting sort, \texttt{nextfloat}/\allowbreak\texttt{prevfloat} as the draft Next operations, \texttt{codepoint}, and \texttt{rand}/\allowbreak\texttt{randn} (see the \hyperlinkref{10272851679537904033}{User Guide}). All of it goes through the same projection engine and decode tables as the draft-named API.
...
\texttt{Binary <: AbstractFloat}, and generic Julia code reaches for that interface without asking. The whole of it is supplied:
...
\texttt{widen} & the format{\textquotesingle}s promotion carrier — see \href{../other/promoterules.md}{the promotion study} for why it is not another format \\
...
κ is the maximum code-point distance (along the total order) between an implementation{\textquotesingle}s result and the defined result, over inputs with finite defined results; any mismatch on non-finite defined results makes κ = NaN. Because inputs are enumerable, \texttt{register\_\allowbreak{}approx!} \emph{measures} κ exhaustively at registration and rejects understatement — a declared bound is a verified property, not a promise. \texttt{conformance()} assembles the declaration live from the operation registry, the table cache, and the approximation registry.
...
\emph{All numbers from this machine/run; absolute values vary by host. Regenerate with \texttt{julia --project=benchmark benchmark/\allowbreak{}benchmarking.jl}.}
