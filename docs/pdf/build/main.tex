...
\toprule
nearest, ties even & \texttt{RNE\_\allowbreak{}SatFinite} & \texttt{RNE\_\allowbreak{}SatPropagate} & \texttt{RNE\_\allowbreak{}SatNone} \\
nearest, ties away & \texttt{RNA\_\allowbreak{}SatFinite} & \texttt{RNA\_\allowbreak{}SatPropagate} & \texttt{RNA\_\allowbreak{}SatNone} \\
toward +∞ & \texttt{RTP\_\allowbreak{}SatFinite} & \texttt{RTP\_\allowbreak{}SatPropagate} & \texttt{RTP\_\allowbreak{}SatNone} \\
toward −∞ & \texttt{RTN\_\allowbreak{}SatFinite} & \texttt{RTN\_\allowbreak{}SatPropagate} & \texttt{RTN\_\allowbreak{}SatNone} \\
toward zero & \texttt{RTZ\_\allowbreak{}SatFinite} & \texttt{RTZ\_\allowbreak{}SatPropagate} & \texttt{RTZ\_\allowbreak{}SatNone} \\
round to odd & \texttt{RTO\_\allowbreak{}SatFinite} & \texttt{RTO\_\allowbreak{}SatPropagate} & \texttt{RTO\_\allowbreak{}SatNone} \\
\bottomrule
\end{tabulary}
...
\texttt{RNE\_\allowbreak{}SatNone} is the package-wide default. Saturation modes mean:
...
\toprule
A & \texttt{RSA\_\allowbreak{}SatFinite(N)} & \texttt{RSA\_\allowbreak{}SatPropagate(N)} & \texttt{RSA\_\allowbreak{}SatNone(N)} \\
B & \texttt{RSB\_\allowbreak{}SatFinite(N)} & \texttt{RSB\_\allowbreak{}SatPropagate(N)} & \texttt{RSB\_\allowbreak{}SatNone(N)} \\
C & \texttt{RSC\_\allowbreak{}SatFinite(N)} & \texttt{RSC\_\allowbreak{}SatPropagate(N)} & \texttt{RSC\_\allowbreak{}SatNone(N)} \\
\bottomrule
\end{tabulary}
...
Common Base spellings under \texttt{RNE\_\allowbreak{}SatNone}:
...
\toprule
\texttt{NearestTiesToEven} & \texttt{RNE\_\allowbreak{}SatFinite} & \texttt{RNE\_\allowbreak{}SatPropagate} & \texttt{RNE\_\allowbreak{}SatNone} \\
\texttt{NearestTiesToAway} & \texttt{RNA\_\allowbreak{}SatFinite} & \texttt{RNA\_\allowbreak{}SatPropagate} & \texttt{RNA\_\allowbreak{}SatNone} \\
\texttt{TowardPositive} & \texttt{RTP\_\allowbreak{}SatFinite} & \texttt{RTP\_\allowbreak{}SatPropagate} & \texttt{RTP\_\allowbreak{}SatNone} \\
\texttt{TowardNegative} & \texttt{RTN\_\allowbreak{}SatFinite} & \texttt{RTN\_\allowbreak{}SatPropagate} & \texttt{RTN\_\allowbreak{}SatNone} \\
\texttt{TowardZero} & \texttt{RTZ\_\allowbreak{}SatFinite} & \texttt{RTZ\_\allowbreak{}SatPropagate} & \texttt{RTZ\_\allowbreak{}SatNone} \\
\texttt{ToOdd} & \texttt{RTO\_\allowbreak{}SatFinite} & \texttt{RTO\_\allowbreak{}SatPropagate} & \texttt{RTO\_\allowbreak{}SatNone} \\
\bottomrule
\end{tabulary}
...
\texttt{DefaultProjection} & \texttt{RNE\_\allowbreak{}SatNone} & a \texttt{ProjSpec}, or \texttt{(mode, sat)} \\
...
Code that must be insensitive to the session default names its projection: \texttt{Multiply(\allowbreak{}T, RNE\_\allowbreak{}SatNone, x, y)}.
...
Project it back explicitly when that is what you mean: \texttt{Convert(\allowbreak{}Binary8p4se, RNE\_\allowbreak{}SatNone, decode(\allowbreak{}x) + 2.0)}.
...
\texttt{order\_\allowbreak{}key} & 1.6 ns & 1.4 ns & 0 \\
...
\texttt{project (\allowbreak{}StochasticA[8], R drawn)} & 7.0 ns & 1.9 ns & 0 \\
\bottomrule
\end{tabulary}
...
\toprule
\texttt{vmap unary (\allowbreak{}table gather), n=65536} & 8.71 μs & 8.59 μs & 0 & 0.13 ns/elem — 7.52 Gelem/s \\
\texttt{vmap binary (\allowbreak{}table gather), n=65536} & 16.78 μs & 16.48 μs & 0 & 0.26 ns/elem — 3.91 Gelem/s \\
\texttt{vmap ternary (\allowbreak{}scalar loop), n=65536} & 1.01 ms & 1.0 ms & 0 & 15.49 ns/elem — 0.06 Gelem/s \\
\texttt{vmap binary stochastic (\allowbreak{}scalar loop), n=65536} & 646.35 μs & 633.53 μs & 0 & 9.86 ns/elem — 0.1 Gelem/s \\
...
operation & median & min & allocs & per element \\
\toprule
\texttt{FMA K=4 (\allowbreak{}eager table), n=65536} & 20.47 μs & 19.51 μs & 0 & 0.31 ns/elem — 3.2 Gelem/s \\
\texttt{FMA K=4 (\allowbreak{}eager table), scalar-loop baseline, n=65536} & 967.64 μs & 958.72 μs & 0 & 14.76 ns/elem — 0.07 Gelem/s \\
\texttt{FMA K=6 (\allowbreak{}eager table), n=65536} & 24.75 μs & 24.35 μs & 0 & 0.38 ns/elem — 2.65 Gelem/s \\
\texttt{FMA K=6 (\allowbreak{}eager table), scalar-loop baseline, n=65536} & 1.02 ms & 997.1 μs & 0 & 15.56 ns/elem — 0.06 Gelem/s \\
\texttt{FMA K=8 (\allowbreak{}compute), n=65536} & 1.0 ms & 986.96 μs & 0 & 15.28 ns/elem — 0.07 Gelem/s \\
\texttt{FMA K=8 (\allowbreak{}compute), scalar-loop baseline, n=65536} & 994.01 μs & 982.32 μs & 0 & 15.17 ns/elem — 0.07 Gelem/s \\
\bottomrule
\end{tabulary}
...
operation & median & min & allocs \\
\toprule
\texttt{sort! (\allowbreak{}counting sort via defalg), n=65536} & 156.82 μs & 154.99 μs & 2 \\
\texttt{sort! (\allowbreak{}stock comparison sort), n=65536} & 1.39 ms & 1.35 ms & 3 \\
\texttt{sort! rev=true (\allowbreak{}counting sort), n=65536} & 160.84 μs & 158.14 μs & 2 \\
\bottomrule
\end{tabulary}
...
Cold cache per sample (\texttt{empty\_\allowbreak{}tables!} in untimed setup); JIT pre-warmed. The warm-hit column is the steady-state cost of \texttt{get\_\allowbreak{}table} when the specialization is already cached (median / min). Table entries enumerate every code point by construction (NaN and ±Inf included).
...
\texttt{Tanh⟨8p4se⟩ (\allowbreak{}256 entries)} & 29.85 μs & 29.1 μs & 257 & 64.3 ns / 62.6 ns \\
\texttt{Add⟨8p4se×8p4se⟩ (\allowbreak{}64 K entries)} & 25.49 ms & 23.51 ms & 785668 & 64.4 ns / 62.7 ns \\
\texttt{Divide⟨8p4se×8p4se⟩ (\allowbreak{}64 K)} & 25.24 ms & 23.2 ms & 784906 & 63.3 ns / 61.7 ns \\
\texttt{Add⟨8p1uf×8p1uf⟩ (\allowbreak{}64 K, wide-spread)} & 42.65 ms & 42.06 ms & 1587030 & 64.3 ns / 62.7 ns \\
\bottomrule
\end{tabulary}
...
\texttt{BlockDotProduct → 8p4se (\allowbreak{}B=32)} & 294.0 ns & 220.0 ns & 3 & 9.19 ns/lane \\
\texttt{BlockReduceAdd → 8p4se (\allowbreak{}B=32)} & 457.8 ns & 21.8 ns & 0 & 14.3 ns/lane \\
\texttt{ConvertToBlockMaxAbsFinite (\allowbreak{}B=32)} & 1.84 μs & 1.75 μs & 75 & 57.55 ns/lane \\
\bottomrule
\end{tabulary}
...
\texttt{T(\allowbreak{}::Float64) numeric constructor (\allowbreak{}projects)} & 7.6 ns & 2.8 ns & 0 &  \\
\texttt{Convert 8p4se → 8p3se (\allowbreak{}scalar)} & 6.6 ns & 1.8 ns & 0 &  \\
\texttt{Float64 → 8p4se (\allowbreak{}project)} & 6.5 ns & 1.8 ns & 0 &  \\
\texttt{PackedVector pack, n=65536} & 33.21 μs & 32.42 μs & 3 & 0.51 ns/elem \\
\texttt{PackedVector unpack (\allowbreak{}collect), n=65536} & 28.99 μs & 28.87 μs & 3 & 0.44 ns/elem \\
\bottomrule
\end{tabulary}
...
The \texttt{::Float64} assertions are load-bearing: lane decodes are exact by width (≤ 17-bit significands), and the assertion turns that analysis into a runtime check. The \texttt{Add} result may be any kind in the protocol — \texttt{\_\allowbreak{}bp\_\allowbreak{}element} finishes all of them. Dividing by a scale of \texttt{1.0} is the identity fast path.
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{8725867475956632017}{\texttt{SmallFloats.RNA\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{16426343133939897049}{\texttt{SmallFloats.RNA\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{9948955282984860181}{\texttt{SmallFloats.RNA\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{3359638186361138749}{\texttt{SmallFloats.RNE\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{6254846941289803954}{\texttt{SmallFloats.RNE\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{15613002957007008711}{\texttt{SmallFloats.RNE\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{6431776125964272887}{\texttt{SmallFloats.RTN\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{18308597707086069238}{\texttt{SmallFloats.RTN\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{15860643992407021701}{\texttt{SmallFloats.RTN\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{1023017805428823485}{\texttt{SmallFloats.RTO\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{16442577991271067062}{\texttt{SmallFloats.RTO\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{15307190884920783579}{\texttt{SmallFloats.RTO\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{7512419803180778431}{\texttt{SmallFloats.RTP\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{11481809423421058656}{\texttt{SmallFloats.RTP\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{5630752953060625376}{\texttt{SmallFloats.RTP\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{8937139283316265274}{\texttt{SmallFloats.RTZ\_\allowbreak{}SatFinite}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{569534270503185777}{\texttt{SmallFloats.RTZ\_\allowbreak{}SatNone}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{11314891540121714893}{\texttt{SmallFloats.RTZ\_\allowbreak{}SatPropagate}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
The session{\textquotesingle}s default projection specification. Initialized to \texttt{(\allowbreak{}DefaultRoundingMode(\allowbreak{}), DefaultSaturationMode(\allowbreak{}))} = \texttt{RNE\_\allowbreak{}SatNone}. Setting it directly decomposes ρ into \hyperlinkref{790339220741924172}{\texttt{DefaultRoundingMode}} and \hyperlinkref{11118232298850481653}{\texttt{DefaultSaturationMode}}, so the three stay coherent in both directions.
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{590311556486812861}{\texttt{SmallFloats.conformance\_\allowbreak{}dict}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{18172896664359196039}{\texttt{SmallFloats.conformance\_\allowbreak{}report}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
Implemented as a constant-tuple lookup (bitops plan K2): the per-format table is generated once from \texttt{\_\allowbreak{}decode\_\allowbreak{}compute} above, so the two are correct by construction and asserted equivalent exhaustively; constant inputs still fold.
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{14130930211625684600}{\texttt{SmallFloats.empty\_\allowbreak{}tables!}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{17243710630823570067}{\texttt{SmallFloats.f32\_\allowbreak{}exact}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{9290299928136503291}{\texttt{SmallFloats.f32\_\allowbreak{}impl}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{17132358657286871632}{\texttt{SmallFloats.ftz\_\allowbreak{}variant}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{2984505828320663690}{\texttt{SmallFloats.list\_\allowbreak{}approx}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{14020185040969508771}{\texttt{SmallFloats.measure\_\allowbreak{}kappa}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{3878213895374713375}{\texttt{SmallFloats.register\_\allowbreak{}approx!}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{11323396924847339952}{\texttt{SmallFloats.register\_\allowbreak{}f32!}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{10680873580113776002}{\texttt{SmallFloats.table\_\allowbreak{}bytes}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{12365808433851041199}{\texttt{SmallFloats.unregister\_\allowbreak{}approx!}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{10552114894679459838}{\texttt{SmallFloats.with\_\allowbreak{}default\_\allowbreak{}accumulatortype}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{16240126272133078948}{\texttt{SmallFloats.with\_\allowbreak{}default\_\allowbreak{}projection}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{5780687441881078027}{\texttt{SmallFloats.with\_\allowbreak{}default\_\allowbreak{}returntype}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{7019104179409499666}{\texttt{SmallFloats.with\_\allowbreak{}default\_\allowbreak{}type}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{307234055475539643}{\texttt{SmallFloats.DEFAULT\_\allowbreak{}RBITS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{6952275749280347714}{\texttt{SmallFloats.NAN\_\allowbreak{}ORDER\_\allowbreak{}KEY}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{364714985955516448}{\texttt{SmallFloats.TERNARY\_\allowbreak{}ADAPTIVE\_\allowbreak{}BITS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{3202729790529991172}{\texttt{SmallFloats.TERNARY\_\allowbreak{}BUILD\_\allowbreak{}ELEMS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{7367898474622902585}{\texttt{SmallFloats.TERNARY\_\allowbreak{}CACHE\_\allowbreak{}BYTES}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{13776331714046560855}{\texttt{SmallFloats.TERNARY\_\allowbreak{}EAGER\_\allowbreak{}BITS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{16271046097526486057}{\texttt{SmallFloats.THREADED\_\allowbreak{}KERNELS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{8230965136284310108}{\texttt{SmallFloats.THREAD\_\allowbreak{}MIN\_\allowbreak{}ELEMS}}  -- {Constant.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
Construct a value from its \textbf{code point} (validated: for K < 8, \texttt{c} must be \texttt{< 2{\textasciicircum}K} or an \texttt{ArgumentError} is thrown; the range check folds away at K = 8 and for constant codes). \texttt{Binary5p3sf(\allowbreak{}0x08) == Binary5p3sf(\allowbreak{}1.0)}.
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{17928462762263157549}{\texttt{SmallFloats.\_\allowbreak{}bp\_\allowbreak{}element}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{15953021384962181472}{\texttt{SmallFloats.\_\allowbreak{}check\_\allowbreak{}tabulable}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{12008461486952181445}{\texttt{SmallFloats.\_\allowbreak{}comparable}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{7363307359868208211}{\texttt{SmallFloats.\_\allowbreak{}resolve\_\allowbreak{}rng}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{13712544213067904585}{\texttt{SmallFloats.\_\allowbreak{}rng\_\allowbreak{}for}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{17911291590751857121}{\texttt{SmallFloats.\_\allowbreak{}ternary\_\allowbreak{}table\_\allowbreak{}for}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{8539869125382303808}{\texttt{SmallFloats.apply\_\allowbreak{}op}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}

apply\_\allowbreak{}op(Val(op), fr, ρ, R, xs...) — evaluate \texttt{op}{\textquotesingle}s ω-semantics on decoded Float64 operands and project the result into \texttt{fr} under ρ (with random bits R).
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{10413485981529569906}{\texttt{SmallFloats.get\_\allowbreak{}table}}  -- {Function.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{7588185958709694166}{\texttt{SmallFloats.get\_\allowbreak{}table}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{1151381039512186337}{\texttt{SmallFloats.project\_\allowbreak{}interval}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
...
Project the true value of \texttt{f}, where \texttt{f(prec)} returns an enclosure \texttt{(\allowbreak{}d::BigFloat, u::BigFloat)} with the true value in \texttt{[d, u]} (endpoints from MPFR directed rounding). Rigor (design §9.2): if d == u the value is exact — project it. Otherwise the true value lies in the \emph{open} interval (d, u) (directed correct rounding of a non-representable value moves strictly); projection is monotone in the value for every mode at fixed R, so if project(d, sticky=+1) == project(u, sticky=-1) that common code is the answer; otherwise a projection grid point sits inside the interval and precision escalates.
...
\par\medskip\noindent\begin{minipage}{\linewidth}
\hypertarget{835860055659755424}{\texttt{SmallFloats.unpack\_\allowbreak{}tile!}}  -- {Method.}
\nopagebreak
\begin{adjustwidth}{2em}{0pt}
