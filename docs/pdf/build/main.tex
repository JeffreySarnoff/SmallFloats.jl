...
SmallFloats.jl implements the IEEE P3109 draft{\textquotesingle}s small binary formats for Julia, across bitwidths 3 through 16. Its default operations compute the mathematically exact result and project once into the requested format.
...
The display shows both parts of a stored value: \texttt{1.625} is the exact datum and \texttt{0x45} is the code point that names it. The addition computes the exact sum, 3.375, then applies the named projection \texttt{RNE\_\allowbreak{}SN} once. That relationship—an exact value, a code point, and an explicit projection—is the center of the package and of this manual.
...
\label{16895118486123008921}{}


\subsection{Start using the package}



\label{14557625339855838866}{}


Begin with \href{install\_\allowbreak{}and\_\allowbreak{}verify.md}{Install and Verify}, then work through \href{first\_\allowbreak{}session.md}{First Session}. Read \href{core\_\allowbreak{}model.md}{Core Model} once before moving into a larger workflow. Together these pages take you from installation to the four distinctions that prevent most mistakes.



\subsection{Complete an application task}



\label{9042405658798712021}{}
...
\begin{itemize}
\item \href{workflow\_\allowbreak{}choose\_\allowbreak{}format.md}{Choose a Format}


\item \href{workflow\_\allowbreak{}quantize\_\allowbreak{}measure.md}{Quantize and Measure a Tensor}


\item \href{workflow\_\allowbreak{}rounding\_\allowbreak{}overflow.md}{Control Rounding and Overflow}


\item \href{workflow\_\allowbreak{}arrays.md}{Run Operations over Arrays}


\item \href{workflow\_\allowbreak{}blocks.md}{Use Blocks for Dynamic Range}


\item \href{workflow\_\allowbreak{}stochastic.md}{Make Stochastic Work Reproducible}


\item \href{workflow\_\allowbreak{}packed\_\allowbreak{}storage.md}{Pack Values for Storage}


\item \href{workflow\_\allowbreak{}float16.md}{Interoperate with Float16 and BFloat16}
...
\subsection{Understand the semantics}



\label{11823565148484650444}{}


\href{concept\_\allowbreak{}p3109.md}{P3109 in One Chapter} introduces the draft standard. \href{concept\_\allowbreak{}format\_\allowbreak{}anatomy.md}{Format Anatomy}, \href{concept\_\allowbreak{}values\_\allowbreak{}codepoints.md}{Values, Code Points, and Conversion}, and \href{concept\_\allowbreak{}exact\_\allowbreak{}then\_\allowbreak{}project.md}{The Exact-Then-Project Contract} form the conceptual spine. The remaining concept pages explain Julia integration, session state, performance, and the difference between defined, stochastic, and approximate results.



\subsection{Look up a contract}



\label{8485665184536350380}{}


Reference pages are organized by API family. Start with \href{reference\_\allowbreak{}formats\_\allowbreak{}values.md}{Formats and Value Queries}, \href{reference\_\allowbreak{}projections.md}{Projection Specifications}, or the \href{reference\_\allowbreak{}operations.md}{Operation Catalog}. The \href{reference\_\allowbreak{}public\_\allowbreak{}api.md}{Public API Index} collects source docstrings after the curated family references.



\subsection{Inspect evidence or implementation}



\label{1534503083051065441}{}


\href{examples\_\allowbreak{}gallery.md}{Examples and Evidence} indexes complete applied and verification sessions. Maintainers should begin with \href{internals\_\allowbreak{}architecture.md}{Architecture and Invariants}, then follow the data path through encoding, projection, the oracle, tables, and block reductions.



\section{Three promises and one boundary}



\label{4320952253063054218}{}


\subsection{Defined paths are bit-exact}



\label{14536008663510537441}{}
...
\subsection{Policy is visible}



\label{15509295336439983638}{}


Rounding and saturation are values in a \texttt{ProjSpec}. Convenience operators use the session default, while the spec-named register—\texttt{Add(T, ρ, x, y)}, \texttt{Exp(T, ρ, x)}, and so on—makes policy explicit and reproducible.



\subsection{Approximation is declared and measured}



\label{5845490803687023215}{}
...
\subsection{P3109 is not IEEE 754 in smaller storage}



\label{3933450065329860683}{}


P3109 has one NaN and one zero, defines a different bias rule, and makes signedness and finite/extended domain format parameters. In particular, \texttt{Binary16p11se} is not \texttt{Float16}, and converting between them is never a reinterpretation. See \href{concept\_\allowbreak{}p3109.md}{P3109 in One Chapter} before porting IEEE-specific assumptions.



\section{The rule to remember first}



\label{2041641093961892260}{}


An \texttt{Unsigned} constructor argument is a \textbf{code point}. Every other \texttt{Real} constructor argument is a \textbf{value to project}.
...
Use \texttt{codepoint(x)} and \texttt{decode(x)} to make the two interpretations explicit.
...
\label{4150296715313959346}{}


The package implements all 504 legal formats at \texttt{K ∈ 3:16}. \texttt{using SmallFloats} exports the 120 aliases at \texttt{K ≤ 8}; the other 384 are available through \texttt{SmallFloats.Binary…}, \texttt{format(K, P, SGN, EXT)}, or \texttt{using SmallFloats.Formats}.
...
\href{install\_\allowbreak{}and\_\allowbreak{}verify.md}{Install and Verify} → \href{first\_\allowbreak{}session.md}{First Session} → \href{core\_\allowbreak{}model.md}{Core Model}.
...
\href{first\_\allowbreak{}session.md}{First Session}.
...
This session establishes the package{\textquotesingle}s working vocabulary and runs one scalar and one array operation. It assumes \href{install\_\allowbreak{}and\_\allowbreak{}verify.md}{Install and Verify} has succeeded.
...
\texttt{Binary8p4se} is an 8-bit, precision-4, signed, extended format. It does not contain the number 1.6. Construction therefore projects 1.6 to the nearest datum, 1.625, and stores its code point, \texttt{0x45}.
...
An \texttt{Unsigned} argument means \textbf{code point}, not numeric value. Compare the two forms:
...
Use an unsigned integer when you mean stored identity. Use a signed integer or other \texttt{Real} when you mean a value to project. \texttt{decode} and \texttt{codepoint} recover the two views:
...
The exact sum is 3.375. \texttt{RNE\_\allowbreak{}SN} says how that exact result lands in \texttt{Binary8p4se}: round to nearest with ties to even, then apply the draft{\textquotesingle}s \texttt{SatNone} boundary rules. The result is 3.5 at code point \texttt{0x4e}.
...
Use \texttt{+} for exploratory same-format arithmetic. Use \texttt{Add(T, ρ, ...)} in code whose policy must be visible and independent of session state.
...
Both calls compute the exact product 400. \texttt{RNE\_\allowbreak{}SN} produces infinity for this case; \texttt{RNE\_\allowbreak{}SF} clamps to the largest finite datum. The projection is part of the operation{\textquotesingle}s meaning.
...
They do not. The first selects code point \texttt{0x48}, whose datum is 2.0. The second projects the numeric value 72 into the format.
...
\label{16604699920213877893}{}


\begin{itemize}
\item a datum is an exact member of a format;


\item a code point names that datum;


\item an unsigned constructor argument selects a code point;


\item computation produces an exact result and projects once;


\item explicit operation forms name result format and policy;


\item array forms preserve the scalar contract.
...
\href{core\_\allowbreak{}model.md}{Core Model}.
...
\href{workflow\_\allowbreak{}choose\_\allowbreak{}format.md}{Choose a Format} or \href{workflow\_\allowbreak{}quantize\_\allowbreak{}measure.md}{Quantize and Measure a Tensor}.



\subsection{Look it up}



\label{9973953341584746313}{}
...
This is why the \texttt{Unsigned}-means-code-point rule (from the First Session) exists: \texttt{codepoint} and the constructor are inverses \emph{by design} —
...
\texttt{x + y} from the First Session: exact sum 3.375, projected once to 3.5. \texttt{200 × 2}: exact product 400 (not representable — projection decides between \texttt{Inf}, \texttt{224}, or \texttt{NaN}, and the spec says which).
...
The package{\textquotesingle}s speed comes from the first idea{\textquotesingle}s finiteness: an 8-bit unary operation is a function with 256 inputs, so its \emph{entire} behavior is a 256-byte table, built once through the exact scalar path and then gathered per element (0.13 ns). The exactness is inherited: table and scalar are the same function by construction.
...
\href{workflow\_\allowbreak{}choose\_\allowbreak{}format.md}{Choose a Format} applies the model to an application decision. \href{concept\_\allowbreak{}values\_\allowbreak{}codepoints.md}{Values, Code Points, and Conversion} develops stored identity in more detail.
...
\label{9973953341584746313-dup1}{}


\href{reference\_\allowbreak{}formats\_\allowbreak{}values.md}{Formats and Value Queries}, \href{reference\_\allowbreak{}projections.md}{Projection Specifications}, and \href{reference\_\allowbreak{}conformance.md}{Conformance and Approximation Registry}.
...
\href{workflow\_\allowbreak{}packed\_\allowbreak{}storage.md}{Pack Values for Storage} or \href{workflow\_\allowbreak{}blocks.md}{Use Blocks for Dynamic Range}.
...
\label{9973953341584746313-dup2}{}
...
Use this workflow to apply the scalar operation contract to arrays, choose between named calls, \texttt{vmap!}, and an \texttt{Op} callable, and avoid measuring a cold table build as steady-state performance.
...
Use this workflow when groups of values need more dynamic range than a narrow element format can provide. A \texttt{Block} pairs one shared scale with a group of elements, following the MX-style \texttt{scale × element} model.
...
\label{13405352499289318804-dup1}{}
...
Out-of-range codes throw for K < 8 (\texttt{Binary5p3sf(0x20)} is an error). Round-tripping is \texttt{T(codepoint(x)) === x}. The unchecked constructor is an internal kernel primitive rather than part of the exported interface.
...
\label{13405352499289318804-dup2}{}


\href{concept\_\allowbreak{}rounding\_\allowbreak{}saturation.md}{Rounding and Saturation} and \href{reference\_\allowbreak{}formats\_\allowbreak{}values.md}{Formats and Value Queries}.
...
\label{13405352499289318804-dup3}{}


\href{concept\_\allowbreak{}rounding\_\allowbreak{}saturation.md}{Rounding and Saturation} develops the two policy axes. \href{workflow\_\allowbreak{}rounding\_\allowbreak{}overflow.md}{Control Rounding and Overflow} applies them to midpoint and boundary cases.
...
A projection answers two independent questions. Rounding chooses a neighbor when an exact value lies between datums. Saturation chooses the permitted result when the rounded value lies outside the format{\textquotesingle}s range or belongs to a special boundary case.
...
SmallFloats represents the pair as \texttt{ProjSpec(\allowbreak{}rounding, saturation)}. Names such as \texttt{RNE\_\allowbreak{}SN} are convenient values, not fused algorithms. The independence is important: changing saturation must not silently change how in-range values round, and changing rounding can alter which side of a boundary is reached.
...
Stochastic modes make the landing a function of both the exact fractional position and a finite random draw. They are defined distributions. With an explicit draw \texttt{R}, a stochastic projection is deterministic and can be tested exhaustively.
...
\href{workflow\_\allowbreak{}rounding\_\allowbreak{}overflow.md}{Control Rounding and Overflow} and \href{workflow\_\allowbreak{}stochastic.md}{Make Stochastic Work Reproducible}.
...
\label{9973953341584746313-dup3}{}
...
\label{13405352499289318804-dup4}{}
...
Five values control what {\textquotedbl}the default{\textquotedbl} means for any convenience call in SmallFloats.jl — the format \texttt{T(2.1)} produces, the projection \texttt{x + y} follows, and the result format an omitted result type selects. This page explains what those five defaults are, how they stay consistent with each other, why they are dangerous to set casually, and the safe pattern for consuming them.
...
\label{13405352499289318804-dup5}{}
...
The table-gather kernels that back array operations run roughly 25× faster than the scalar path per element, because after the first call for a given \texttt{(op, formats, ρ)} specialization, every later element is a single table lookup rather than a full evaluation. That first call pays a one-time table build: roughly 6 µs for a 256-entry unary table, 80 µs for a 64 K-entry 8×8 binary table, and a few milliseconds for a wide-spread signature whose entries escalate to MPFR. Benchmark warm calls separately from the first cold call, or the build cost will dominate a measurement that is supposed to be about steady-state throughput. Once warm, measured throughput is around 0.13 ns/element for unary gathers and 0.25 ns/element for binary gathers.



Builds are also \textbf{parallel} when the table is large enough and the process has threads, so the first-call cost above is what one core pays; see \href{examples\_\allowbreak{}performance.md}{Performance Evidence} for the measured contrast.
...
\item \textbf{Compute} (\texttt{K = 8}; a 16 MiB table stops being a cache win): the scalar pipeline runs per element instead, threaded for long arrays.

\end{itemize}


\subsubsection{Wide formats take the compute path, and it threads}



\label{742507674469878187}{}


Above \texttt{K = 8} the same reasoning reaches unary and binary operations. A binary table for two \texttt{K = 16} operands would be \texttt{2{\textasciicircum}32} entries, so the policy declines it and the \textbf{compute kernel is the only array path those formats have}. It is threaded for long arrays under a pure ρ, at every arity — measured at roughly 6× on 8 threads for a 65 536-element \texttt{Binary16p8se} \texttt{Add}.



Stochastic ρ is deliberately excluded: it draws from a single stream, so its results are reproducible only in index order, and threading it would make a seeded run depend on the scheduler.
...
Sorting is special-cased rather than falling through to Base{\textquotesingle}s generic comparison sort. \texttt{Binary} values compare through integer order keys, and a vector of \texttt{Binary} values sorts with an \textbf{O(n) counting sort} installed as the default algorithm — roughly 8× the stock comparison sort at 64K elements, with \texttt{rev=true} supported at the same cost. \texttt{sort(A)} simply picks this path up automatically. P3109{\textquotesingle}s total order places the single NaN first (last under \texttt{rev=true}), unlike Base{\textquotesingle}s ordering for ordinary floating-point NaNs.
...
\label{13405352499289318804-dup6}{}
...
\label{7476714143305412830-dup2}{}
...
\label{9973953341584746313-dup4}{}
...
\label{13405352499289318804-dup7}{}
...
\subsection{Related contracts}



\label{13057688750462646042}{}
...
\label{13057688750462646042-dup1}{}
...
\label{13057688750462646042-dup2}{}
...
\label{13057688750462646042-dup3}{}
...
\label{13057688750462646042-dup4}{}
...
\label{13057688750462646042-dup5}{}
...
\hyperlinkref{12405290678054954100}{Activations are 256-byte tables} & a whole unary op \emph{is} its table, at a fraction of a nanosecond per element \\
...
\hyperlinkref{585007679975857547}{Benchmarking without measuring the dispatcher} & the two measurement post-mortems this package learned from \\
\bottomrule
\end{tabulary}
...
\label{7476714143305412830-dup3}{}
...
\subsection{Continue through the implementation}



\label{2948775379247563076}{}
...
\label{9862256287407127426-dup5}{}


\href{internals\_\allowbreak{}projection\_\allowbreak{}engine.md}{Projection Engine}.
...
\texttt{sticky ∈ \{-1,0,+1\}} carries that relation into every rounding comparison. The delicate “just below a datum” case crosses into the preceding binade and uses a fraction of \texttt{1⁻}; treating it as the endpoint itself would give the wrong answer for directed modes and ties.
...
\label{9862256287407127426-dup6}{}


\href{internals\_\allowbreak{}oracle.md}{Oracle and Rigor Classes}.
...
\label{2948775379247563076-dup1}{}
...
\label{5962693725618524284}{}


For pure specs, unary and binary operations are \textbf{finite functions} — 256 or 65 536 entries — so the kernel layer materializes them once per \texttt{(op, formats, ρ)} into a locked cache (\texttt{Dict\{TableKey, Memory\{UInt8\}\}}, double-checked locking, builds outside the lock) and serves every later array call as a gather: Shape-A, one load per element, measured 0.13 ns/elem unary and 0.25 ns/elem binary. Tables are built \emph{through the scalar path}, so they inherit its bit-exactness; the suite asserts table ≡ scalar over every entry.



Builds are \textbf{parallel} above \texttt{TABLE\_\allowbreak{}BUILD\_\allowbreak{}MIN\_\allowbreak{}ENTRIES}, partitioned over the outer code loop so each task writes a contiguous, disjoint span of the preallocated \texttt{Memory}. Nothing about the cache changes: the build already ran outside the lock, with a racing duplicate build documented as benign. Only pure ρ is ever tabulated, \texttt{\_\allowbreak{}scalar\_\allowbreak{}code} holds no mutable state, and MPFR escalation reaches \texttt{setprecision}/\allowbreak\texttt{setrounding} through their \emph{function} forms, which are \texttt{ScopedValue}-backed on Julia 1.12 and therefore task-local — the three facts the parallelism rests on, and the reason the package floors at 1.12.



The fill loops are separate functions (\texttt{\_\allowbreak{}fill\_\allowbreak{}unary!}, \texttt{\_\allowbreak{}fill\_\allowbreak{}binary!}, \texttt{\_\allowbreak{}fill\_\allowbreak{}ternary!}) taking \texttt{Val(op)} as an \textbf{argument}, and that is load-bearing rather than tidiness. \texttt{\_\allowbreak{}build\_\allowbreak{}*} receives \texttt{op} as a runtime \texttt{Symbol}, so a \texttt{Val(op)} constructed in the same function body is type-unstable and every entry pays a dynamic dispatch; passing it across a barrier makes \texttt{op} a type parameter of the callee and specializes the whole scalar path. Measured on a 64 K-entry \texttt{Multiply} table: \textbf{445 µs → 64 µs}.
...
\item \textbf{Compute} (\texttt{K = 8}, 16+ MiB — a table is never worth it): \texttt{vmap!} runs Shape-B, the fully specialized scalar pipeline per element.
...
\subsubsection{Shape-B threads at every arity}



\label{4450971302404912645}{}


\texttt{Threads.@threads} is a property of the \textbf{pure-ρ compute loop}, not of the ternary tier that first needed it. \texttt{\_\allowbreak{}vmap\_\allowbreak{}compute!} covers unary and binary as well, gated by \texttt{THREADED\_\allowbreak{}KERNELS} and \texttt{THREAD\_\allowbreak{}MIN\_\allowbreak{}ELEMS} through the single \texttt{\_\allowbreak{}should\_\allowbreak{}thread} predicate. Above \texttt{K = 8} this is the case that matters: a binary table would be \texttt{2{\textasciicircum}(K1+K2)} entries, \texttt{table\_\allowbreak{}for} declines, and Shape-B is the only array path the format has — measured at roughly 6× on 8 threads for a 65 536-element \texttt{Binary16p8se} \texttt{Add}.



Stochastic ρ stays sequential, and that requirement is carried by \emph{which function you are in} rather than by a branch: \texttt{\_\allowbreak{}vmap\_\allowbreak{}scalar!} keeps the stochastic traffic and never threads, because one rng stream is reproducible only in index order. The suite pins threaded and sequential results as \textbf{identical code points in identical order}, not as agreement within a tolerance — a threading change that altered a result is a defect no timing would reveal.
...
\label{2948775379247563076-dup2}{}


\href{internals\_\allowbreak{}encoding\_\allowbreak{}decoding.md}{Encoding and Decoding}, \href{internals\_\allowbreak{}oracle.md}{Oracle and Rigor Classes}, \href{examples\_\allowbreak{}verification.md}{Verification Sessions}.
...
\label{13106405904108779198-dup2}{}
...
\label{2948775379247563076-dup3}{}
...
\item \href{workflow\_\allowbreak{}blocks.md}{Working with Blocks} — the task-oriented guide.


\item \href{workflow\_\allowbreak{}blocks.md}{Blocks for Dynamic Range} — when to reach for one.
...
Recorded after two measurement post-mortems: a benchmark closure over any non-\texttt{const} global measures Julia{\textquotesingle}s dispatch machinery, not the code under test — and it distorts \emph{ratios}, not just absolutes, because dispatch cost varies with call shape.
...
\label{2948775379247563076-dup4}{}
...
\label{2948775379247563076-dup5}{}
...
\label{2948775379247563076-dup6}{}
...
\subsection{Related help}



\label{9520074481071877092}{}
...
\label{9520074481071877092-dup1}{}
...
\label{9520074481071877092-dup2}{}
