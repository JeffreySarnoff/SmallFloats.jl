# Formal Foundations: the Code-Point Algebra
...
It is the layer *below* [Formats, Aliases & Representations](explain_formats.md).
...
!!! note "Verified, not transcribed"
    ...
    **every positive finite code point** of every format. The check is the
    script in [Worked Examples: Internals](examples_internals.md); it is the
...
`Float64` sorting does. See [Julia Compatibility](ref_julia_compat.md) for what
...
— is in [Worked Examples: Internals](examples_internals.md).

## See also

- [Formats, Aliases & Representations](explain_formats.md) — what a format is,
  and the type-vs-alias distinction.
- [The Encoding & Projection Engine](under_engine.md) — how the package walks
  this algebra at speed.
- [Why `Binary16p11se` is not `Float16`](explain_float16.md) — the bias
  difference, stated in these terms.
- [Format Queries](ref_formats.md) — the package's accessors for every symbol
  named here.
