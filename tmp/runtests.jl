...
@testset "blocks.jl §8" begin
    ...
    blocks = [Block(rnd(T5), ntuple(_ -> rnd(T5), 4)) for _ in 1:T1]
    bv = BlockVector(blocks)
    @test length(bv) == 10 && all(codepoint.(bv[j].x) == codepoint.(blocks[j].x) && bv[j].s === blocks[j].s for j in 1:T1)
    bv[3] = blocks[7]
...
@testset "rand.jl" begin
    ...
    σ8 = RSA_SatNone(8)
    draws(seed) = [codepoint(rand(Xoshiro(seed), T; projection=σ8)) for _ in 1:T5]
    @test draws(5) == draws(5)
    @test draws(5) != draws(6)
    ndraws(seed) = [codepoint(randn(Xoshiro(seed), T; projection=σ8)) for _ in 1:T5]
